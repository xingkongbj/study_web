...
git checkout -- < 文件 >  #还原文件，只能还原有版本控制文件。可以用于工作区误删文件，恢复使用。只修改工作区，对暂存区无效。如果之前提交过暂存区，那么就恢复到暂存区的版本。工作区记录会丢失。
...
git push -u < 库名 > < 分支名 >  #推送本地仓库的分支到远程仓库的分支，第一次要用-u
...
git branch --set-upstream < 本地分支名 > < 远程库名 >/< 远程分支名 >  #设置pull跟踪
git branch --set-upstream-to=< 远程库名 >/< 远程分支名 > < 本地分支名 >  #设置pull跟踪的另一种语法
